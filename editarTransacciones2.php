...
$resu = mysqli_query($db,"SELECT * FROM `transacciones` WHERE IdTransaccion ='" . $_GET["IdTransaccion"] . "'");
$ro = mysqli_fetch_array($resu);
		...
		switch($Con):
			...
			mysqli_query($db, "UPDATE `apartamentos` SET Deuda= (Deuda - $Cantidad) WHERE NumApartamento=$Num");
			break;
